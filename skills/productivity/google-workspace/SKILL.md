...
1. **Outbound email allowlist (CLI-enforced).** `gmail send`/`gmail reply` only permit
   `james@pakele.ai` or `jamespakele@dcshawaii.org` as recipients. Any other `--to`/`--cc`
   (or, for `gmail reply`, the original message's `From`) is **blocked** — exit 2, no email
   sent. Drafts use the `DRAFT: ` subject prefix so James can distinguish agent drafts.
